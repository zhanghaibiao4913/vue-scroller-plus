<template>
  ...
  >
    <div class="_v-content" :id="contentId">
      <div v-if="type === 'default' && onRefresh" class="pull-to-refresh-layer" :class="{'active': state == 1, 'active refreshing': state == 2}">
      ...
      <div v-if="type === 'JN' && onRefresh" class="jn-pull-to-refresh-layer">
        <div class="img-box">
          <refresh-icon :class="{rotate: state == 2}"></refresh-icon>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      type: { // 参数 => default:插件自带的效果, JN:自定义的效果，不回弹，下拉刷新小球
        type: String,
        default: 'JN'
      },
      infiniteBounce: {
    ...
    mounted () {
      ...
      if (this.type === 'JN' && this.onRefresh) {
        this.pullToRefreshLayer = this.content.getElementsByTagName("div")[0];
      }
      let render = getContentRender(this.content, this.pullToRefreshLayer, (pos) => {
      ...
      this.scroller = new Scroller(render, {
        ...
        infiniteBounce: this.infiniteBounce
      })
      ...
      if (this.onInfinite) {
        this.infiniteTimer = setInterval(() => {
          ...
          if (this.content.offsetHeight > 0 && 
            ...
            // console.log(top, this.content.offsetHeight, this.container.clientHeight)
          }
        }, 10);
      }
      ...
      }, 10); this.resizeTimer = setInterval(() => {
        let {width, height} = contentSize()
        if (width !== content_width || height !== content_height) {
          content_width = width
          content_height = height
...
<style lang="css" scoped>
  ...
  .jn-pull-to-refresh-layer .img-box{
    width: 34px;
    height: 100%;
    border-radius: 50%;
